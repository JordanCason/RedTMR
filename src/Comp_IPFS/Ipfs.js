...
// QmRnw4LMRat6UAqsQwftuJkh3Wrbdc9hpQBaWYPfFu4oL8
// 12207d5a99f603f231d53a4f39d1521f98d2e8bb279cf29bebfd0687dc98458e7f89

IPFS.prototype.hexToBase58 = (data) => {
    return new Promise((resolve, reject) => {
        const bytes = Buffer.from('1220' + data.slice(2), 'hex')
        const hash = bs58.encode(bytes)
        resolve(hash)
    })
}

IPFS.prototype.base58ToHex = (address) => {
    return new Promise((resolve, reject) => {
        const bytes = bs58.decode(address)
        const hash = '0x' + bytes.toString('hex').substring(4)
        resolve(hash)
    })
}
