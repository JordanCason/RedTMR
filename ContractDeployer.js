...
const deploy = (inputKey, contractDir) => {
    ...
    const output = solc.compile({
        ...
        return {
            'contents': fs.readFileSync(`${contractDir}${path}`).toString()
        }
    ...
    for (let outputKey in output.contracts) {
        ...
        if (outputKey.includes(inputKey)) {
            ...
            web3.eth.getAccounts().then(function(data) {
                ...
                console.log(address)
                contract.deploy({data: bytecode}).send({from: address, gas: 4000000, gasPrice: '2000000'}).then((instance) => {
                    outputToJs(instance.options.address, abi, contractName, contractDir)
                })
            })
        } else {
